...
func (l *loader) conflictKeysForNQuad(nq *api.NQuad) ([]uint64, error) {
	...
	// 存在该谓语并且不存在冲突，live模式下 opt.ludicrousMode 默认为false
...
func (l *loader) makeRequests() {
	...
	// l.reqs 是一个channel 接受另一个 goroutine 写进来的数据
	for req := range l.reqs {

		//fmt.Println(req)
		req.conflicts = l.conflictKeysForReq(&req)

		// 已经存在则往buffer添加，不存在则 request 处理，并且往 l.conflicts 追加
		if l.addConflictKeys(&req) {
			l.request(req)
		} else {
		...
		// req缓冲的数量，当req等于l.opts.bufferSize 的时候，去执行l.request(req) 写入数据
